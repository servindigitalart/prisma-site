...
const $$FilmHero = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const decade = work.year ? `${Math.floor(work.year / 10) * 10}s` : null;
...
const $$FilmSynopsis = createComponent(($$result, $$props, $$slots) => {
  ...
  const countryLabel = countries.length > 0 ? countries.map((c) => isoToName(c)).join(", ") : "cine internacional";
  const yearStr = work.year ? ` (${work.year})` : "";
  const isStreamable = "is_streamable" in work && work.is_streamable;
...
const $$FilmPeople = createComponent(($$result, $$props, $$slots) => {
  ...
  const ROLE_LABELS = {
    director: "Director",
    cinematography: "Dir. Fotograf\xEDa",
    writer: "Guionista",
    editor: "Editor",
    composer: "Compositor",
    actor: "Actor",
    production_design: "Dir. Arte"
  };
...
const $$FilmMetaAccordion = createComponent(($$result, $$props, $$slots) => {
  ...
  const sections = [
    { id: "technical", title: "Detalles T\xE9cnicos", hasContent: !!(duration || languages.length || studios.length || imdbRating) },
    { id: "distribution", title: "Distribuci\xF3n", hasContent: !!(isCriterion || isMubi || isSightAndSound) }
  ];
...
const $$FilmActions = createComponent(async ($$result, $$props, $$slots) => {
  ...
  return renderTemplate(_a$1 || (_a$1 = __template$1(["", '<div class="film-actions"', "", "", "", "", "", ' data-astro-cid-mctugfxd> <!-- Rate --> <div class="film-action film-action--rate" data-astro-cid-mctugfxd> <button class="film-action__btn" id="fa-rate-btn" aria-label="Calificar esta pel\xEDcula" aria-expanded="false" data-astro-cid-mctugfxd> <span class="film-action__icon" aria-hidden="true" data-astro-cid-mctugfxd>\u2605</span> <span class="film-action__label" id="fa-rate-label" data-astro-cid-mctugfxd>Calificar</span> </button> <div class="rating-picker" id="fa-rating-picker" hidden aria-label="Select rating" data-astro-cid-mctugfxd> ', ` <button class="rating-clear" id="fa-rating-clear" aria-label="Borrar calificaci\xF3n" title="Borrar" data-astro-cid-mctugfxd>\u2715</button> </div> </div> <!-- Seen --> <div class="film-action film-action--seen" data-astro-cid-mctugfxd> <button class="film-action__btn" id="fa-seen-btn" aria-label="Marcar como vista" aria-expanded="false" data-astro-cid-mctugfxd> <span class="film-action__icon" aria-hidden="true" data-astro-cid-mctugfxd>\u2713</span> <span class="film-action__label" data-astro-cid-mctugfxd>Vista</span> </button> <div class="seen-picker" id="fa-seen-picker" hidden data-astro-cid-mctugfxd> <button class="seen-option" id="fa-seen-today" data-astro-cid-mctugfxd>Reci\xE9n vista</button> <button class="seen-option" id="fa-seen-before" data-astro-cid-mctugfxd>La vi antes</button> </div> </div> <!-- Watchlist --> <button class="film-action__btn film-action--watchlist" id="fa-watchlist-btn" aria-label="Agregar a watchlist" data-astro-cid-mctugfxd> <span class="film-action__icon" aria-hidden="true" id="fa-watchlist-icon" data-astro-cid-mctugfxd>+</span> <span class="film-action__label" id="fa-watchlist-label" data-astro-cid-mctugfxd>Watchlist</span> </button> <!-- Review (visible after rating) --> <div class="film-action film-action--review" id="fa-review-wrapper" hidden data-astro-cid-mctugfxd> <button class="film-action__btn" id="fa-review-btn" aria-label="Escribir rese\xF1a" data-astro-cid-mctugfxd> <span class="film-action__icon" aria-hidden="true" data-astro-cid-mctugfxd>\u270E</span> <span class="film-action__label" id="fa-review-label" data-astro-cid-mctugfxd>Rese\xF1a</span> </button> <div class="review-editor" id="fa-review-editor" hidden data-astro-cid-mctugfxd> <textarea id="fa-review-textarea" class="review-textarea" maxlength="500" placeholder="Escribe tu rese\xF1a\u2026" data-astro-cid-mctugfxd></textarea> <div class="review-footer" data-astro-cid-mctugfxd> <span class="review-charcount" id="fa-review-charcount" data-astro-cid-mctugfxd>0/500</span> <button class="review-submit" id="fa-review-submit" data-astro-cid-mctugfxd>Guardar rese\xF1a</button> </div> </div> </div> </div> <script>
(function() {
  function initFilmActions() {
    ...
    function updateReviewVisibility() {
      ...
      if (currentRating) {
        ...
        if (reviewLabel) reviewLabel.textContent = currentReview ? 'Editar rese\xF1a' : 'Rese\xF1a';
      } else {
    ...
    if (reviewBtn && reviewEditor && reviewTextarea && reviewCharcount && reviewSubmit) {
      ...
      reviewSubmit.addEventListener('click', async function() {
        ...
        if (!text) return;
        if (text.length > 500) { showToast('Rese\xF1a demasiado larga (m\xE1x 500)'); return; }
        ...
        try {
          ...
          if (response.ok) {
            ...
            if (reviewLabel) reviewLabel.textContent = 'Editar rese\xF1a';
            showToast('Rese\xF1a guardada');
          }
        } catch(e) {
...
const $$FilmReviews = createComponent(($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${reviews.length > 0 && renderTemplate`${maybeRenderHead()}<section class="film-reviews" data-astro-cid-qu2mrlpg><div class="site-container" data-astro-cid-qu2mrlpg><h2 class="film-reviews__title" data-astro-cid-qu2mrlpg>Reseñas</h2><div class="film-reviews__list" data-astro-cid-qu2mrlpg>${reviews.map((r) => {
...
function buildFilmFaqJsonLd(work) {
  const directors = getDirectorNames(work);
  const synopsis = getSynopsis(work);
  const questions = [];
  questions.push({
...
const $$slug = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const faqJsonLd = buildFilmFaqJsonLd(work);
  const SITE = "https://prisma.film";
  const breadcrumbs = [
    { name: "PRISMA", url: SITE },
    { name: "Pel\xEDculas", url: `${SITE}/rankings/films` },
    { name: `${work.title}${work.year ? ` (${work.year})` : ""}`, url: `${SITE}/films/${slug}` }
  ];
  const user = Astro2.locals.user;
