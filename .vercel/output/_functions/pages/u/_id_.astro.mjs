...
const $$id = createComponent(async ($$result, $$props, $$slots) => {
  ...
  })} </div>` : renderTemplate`<p class="section-empty" data-astro-cid-dsbtqrwp>No films in watchlist yet.</p>`, ratingCount, ratings && ratings.filter((r) => r.rating).length > 0 ? renderTemplate`<div class="ranked-list" data-astro-cid-dsbtqrwp> ${ratings.filter((r) => r.rating).sort((a, b) => b.rating - a.rating).map((r) => {
    const work = r.works;
    if (!work) return null;
    const slug = work.id.replace(/^work_/, "");
  ...
  })} </div>` : renderTemplate`<p class="section-empty" data-astro-cid-dsbtqrwp>No ratings yet.</p>`, seenCount, watches && watches.length > 0 ? renderTemplate`<div class="film-grid" data-astro-cid-dsbtqrwp> ${watches.map((watch) => {
    const work = watch.works;
    if (!work) return null;
    const slug = work.id.replace(/^work_/, "");
    const posterUrl = work.tmdb_poster_path ? `https://image.tmdb.org/t/p/w300${work.tmdb_poster_path}` : null;
    return renderTemplate`<a${addAttribute(`/films/${slug}`, "href")} class="film-card" data-astro-cid-dsbtqrwp> ${posterUrl ? renderTemplate`<img${addAttribute(posterUrl, "src")}${addAttribute(work.title, "alt")} class="film-card__poster" loading="lazy" data-astro-cid-dsbtqrwp>` : renderTemplate`<div class="film-card__placeholder" data-astro-cid-dsbtqrwp>${work.title}</div>`} <div class="film-card__meta" data-astro-cid-dsbtqrwp> <div class="film-card__title" data-astro-cid-dsbtqrwp>${work.title}</div> <div class="film-card__year" data-astro-cid-dsbtqrwp>${work.year}</div> </div> </a>`;
  })} </div>` : renderTemplate`<p class="section-empty" data-astro-cid-dsbtqrwp>No films seen yet.</p>`, reviewCount, ratings && ratings.filter((r) => r.review).length > 0 ? renderTemplate`<div class="reviews-list" data-astro-cid-dsbtqrwp> ${ratings.filter((r) => r.review).map((r) => {
    const work = r.works;
    if (!work) return null;
    const slug = work.id.replace(/^work_/, "");
    const posterUrl = work.tmdb_poster_path ? `https://image.tmdb.org/t/p/w92${work.tmdb_poster_path}` : null;
  ...
  })} </div>` : renderTemplate`<p class="section-empty" data-astro-cid-dsbtqrwp>No reviews yet.</p>`) })}`;
