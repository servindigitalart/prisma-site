/* empty css                                     */
import { e as createAstro, f as createComponent, r as renderTemplate, k as renderComponent, m as maybeRenderHead, h as addAttribute } from '../../chunks/astro/server_DZETslqp.mjs';
import 'piccolore';
...
const $$slug = createComponent(async ($$result, $$props, $$slots) => {
  ...
  })} </div> </div> </section>`} <!-- Film poster grid --> <section class="country-films" data-astro-cid-gsg47nhg> <div class="site-container" data-astro-cid-gsg47nhg> <div class="country-films__header" data-astro-cid-gsg47nhg> <h2 class="country-films__heading" data-astro-cid-gsg47nhg>Filmografía</h2> <span class="country-films__count" data-astro-cid-gsg47nhg>${sorted.length} películas</span> </div> <div class="country-films__grid" id="films-grid" data-astro-cid-gsg47nhg> ${sorted.map((film, i) => {
    const filmSlug = film.id.replace(/^work_/, "");
    const initial = film.title.charAt(0).toUpperCase();
    return renderTemplate`<a${addAttribute(`/films/${filmSlug}`, "href")} class="film-pcard"${addAttribute(i, "data-idx")} data-astro-cid-gsg47nhg> <div class="film-pcard__img-wrap" data-astro-cid-gsg47nhg> ${film.tmdb_poster_path ? renderTemplate`<img${addAttribute(`${TMDB_IMG}${film.tmdb_poster_path}`, "src")}${addAttribute(film.title, "alt")} loading="lazy" decoding="async" data-astro-cid-gsg47nhg>` : renderTemplate`<div class="film-pcard__placeholder" data-astro-cid-gsg47nhg><span data-astro-cid-gsg47nhg>${initial}</span></div>`} </div> <div class="film-pcard__info" data-astro-cid-gsg47nhg> <span class="film-pcard__title" data-astro-cid-gsg47nhg>${film.title}</span> ${film.year && renderTemplate`<span class="film-pcard__year" data-astro-cid-gsg47nhg>${film.year}</span>`} </div> </a>`;
  })} </div> ${sorted.length > 40 && renderTemplate`<div class="films-pagination" data-astro-cid-gsg47nhg> <button class="films-pagination__btn" id="films-prev" disabled data-astro-cid-gsg47nhg>← Anterior</button> <span class="films-pagination__info" id="films-page-info" data-astro-cid-gsg47nhg>1 / ${Math.ceil(sorted.length / 40)}</span> <button class="films-pagination__btn" id="films-next" data-astro-cid-gsg47nhg>Siguiente →</button> </div>`} </div> </section> </div> ` }));
